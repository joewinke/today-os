/**
 * /haoqi3 — vanilla scroll choreography for "the console" cut.
 *
 * No dependencies. Two pinned scenes:
 *   procs — the lifecycle as a live PROCESS TABLE: five processes flip
 ...
 *   gate  — the creed as an authorization moment: "Agents propose." prints,
 *           an approval prompt rises with three queued proposals, verdicts
 *           stamp in (two APPROVED, one REFUSED), and "Humans approve."
 *           lands as the key-ack.
 *
...
export const gate: Action<HTMLElement> = (node) => {
  ...
  const setVars = (g: number) => {
    ...
    node.style.setProperty("--gl", smooth(g, 0.04, 0.22).toFixed(4)) // AGENTS PROPOSE.
    node.style.setProperty("--fr", smooth(g, 0.2, 0.34).toFixed(4)) // prompt frame rises
    node.style.setProperty("--gr", smooth(g, 0.7, 0.84).toFixed(4)) // HUMANS APPROVE. (key-ack)
    node.style.setProperty("--gs", smooth(g, 0.87, 0.95).toFixed(4)) // sub-line

    // proposals: staggered arrival into the queue, then verdict stamps
    const starts = [0.36, 0.46, 0.56]
    for (let i = 0; i < chips.length; i++) {
      const s = starts[i] ?? 0.56
      chips[i].style.setProperty("--in", smooth(g, s - 0.1, s).toFixed(3))
      chips[i].style.setProperty("--stamp", smooth(g, s + 0.12, s + 0.2).toFixed(3))
    }
  }
