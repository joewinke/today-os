/**
 * /haoqi2 — content for the second alternate TodayOS landing page ("the machine" cut).
 *
 * Same product story as the main LP (src/routes/+page.svelte) and /haoqi: the copy
 * below reuses the canonical pitch sentences verbatim — no new claims. This file
 * exists only so the page template stays clean.
 */
...
export interface Station {
  ...
  verb: string
  line: string
  /** tiny mono readout shown under the station marker on the track */
  read: string
}

/** FIND · PITCH · CLOSE · RUN · PROVE — the five stations of the machine. */
...
/**
 * Proposals queuing at the spend-cap gate. Labels are lifted from the live demo
 * fixtures (same set the main LP's ticker uses); the refused row is the
 * "1 BLOCKED BY THE SPEND-CAP GATE" fixture the main LP reports.
 */
...
/** Honest-seam readout for the machine HUD — same figures the main LP reports. */
